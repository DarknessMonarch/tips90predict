...
import countryData from "@/app/components/Countries";
import paymentData from "@/app/components/payment.json";
import {
...
export default function Payment() {
  ...
  const goToPayment = (price, plan) => {
    router.push(`payment/${country}?plan=${plan}&price=${price}`, {
      scroll: false,
    });
  };
        ...
        <div key={`payment-section-${index}`} className={styles.paymentSection}>
          ...
          <div className={styles.payCard}>
            ...
            <div className={styles.payCardInner}>
              ...
              <button
                onClick={() => goToPayment(data.monthly, "Monthly")}
                className={styles.chooseButton}
              >
                Choose monthly plan
              </button>
